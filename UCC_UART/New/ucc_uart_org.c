...
/*
 * Probe function - FULL DEBUG VERSION
 * Shows every step of execution with detailed printouts
 */
static int ucc_uart_probe(struct platform_device *ofdev)
{
	struct device_node *np = ofdev->dev.of_node;
	...
	/* ENTRY DEBUG */
	printk("\n==================================================\n");
	printk("UCC UART PROBE STARTED - Device: %s\n", dev_name(&ofdev->dev));
	printk("Device node: %pOF\n", np);
	printk("==================================================\n");

	/*
	 * Determine if we need Soft-UART mode
	 */
	printk("DEBUG: Checking for soft-uart property...\n");
	if (of_find_property(np, "soft-uart", NULL)) {
		dev_dbg(&ofdev->dev, "using Soft-UART mode\n");
		soft_uart = 1;
		printk("DEBUG: soft-uart property FOUND, soft_uart=1\n");
	} else {
		printk("DEBUG: soft-uart property NOT found\n");
	}
	...
	if (soft_uart) {
		...
		printk("DEBUG: Soft-UART mode enabled, checking firmware...\n");
		qe_fw_info = qe_get_firmware_info();
		
		if (qe_fw_info) {
			printk("DEBUG: qe_get_firmware_info returned: %p\n", qe_fw_info);
			printk("DEBUG: Firmware ID: %s\n", qe_fw_info->id);
		} else {
			printk("DEBUG: qe_get_firmware_info returned NULL\n");
		}

		/* Check if the firmware has been uploaded. */
		if (qe_fw_info && strstr(qe_fw_info->id, "Soft-UART")) {
			firmware_loaded = 1;
			printk("DEBUG: Soft-UART firmware already loaded\n");
		} else {
			char filename[32];
			unsigned int soc;
			unsigned int rev_h;
			...
			printk("DEBUG: Firmware not loaded, attempting to load...\n");
			soc = soc_info(&rev_h, &rev_l);
			printk("DEBUG: soc_info: soc=%u, rev_h=%u, rev_l=%u\n", soc, rev_h, rev_l);
			
			if (!soc) {
				dev_err(&ofdev->dev, "unknown CPU model\n");
				printk("ERROR: Unknown CPU model, aborting\n");
				return -ENXIO;
			}
			sprintf(filename, "fsl_qe_ucode_uart_%u_%u%u.bin",
				soc, rev_h, rev_l);
			printk("DEBUG: Firmware filename: %s\n", filename);
			 ...
			 * kernel, then we use it.
			 */
			printk("DEBUG: Calling request_firmware_nowait...\n");
			ret = request_firmware_nowait(THIS_MODULE,
				FW_ACTION_HOTPLUG, filename, &ofdev->dev,
				GFP_KERNEL, &ofdev->dev, uart_firmware_cont);
			printk("DEBUG: request_firmware_nowait returned: %d\n", ret);
			
			if (ret) {
				dev_err(&ofdev->dev,
					"could not load firmware %s\n",
					filename);
				printk("ERROR: Firmware load failed with %d\n", ret);
				return ret;
			}
			printk("DEBUG: Firmware load initiated successfully\n");
		}
	}

	printk("DEBUG: Allocating qe_port structure (%zu bytes)...\n", sizeof(struct uart_qe_port));
	qe_port = kzalloc(sizeof(struct uart_qe_port), GFP_KERNEL);
	if (!qe_port) {
		dev_err(&ofdev->dev, "can't allocate QE port structure\n");
	...
	printk("DEBUG: qe_port allocated at %p\n", qe_port);

	/* Search for IRQ and mapbase */
	printk("DEBUG: Getting resource address from device tree...\n");
	ret = of_address_to_resource(np, 0, &res);
	printk("DEBUG: of_address_to_resource returned: %d\n", ret);
	if (ret) {
		dev_err(&ofdev->dev, "missing 'reg' property in device tree\n");
		printk("ERROR: Missing 'reg' property\n");
		goto out_free;
	}
	
	printk("DEBUG: Resource start: 0x%llx, end: 0x%llx, length: %llu\n", 
	       (unsigned long long)res.start, (unsigned long long)res.end,
	       (unsigned long long)(res.end - res.start + 1));
	
	if (!res.start) {
		dev_err(&ofdev->dev, "invalid 'reg' property in device tree\n");
		printk("ERROR: Resource start is 0\n");
		ret = -EINVAL;
		goto out_free;
	}
	qe_port->port.mapbase = res.start;
	...
	qe_port->last_good_rx_j = jiffies;
         ...
         */
         ...
         printk("UCC UART Base = 0x%llx\n", (unsigned long long)res.start);
         printk("DEBUG: ucc_mode_param = %d, UCC_UART_SYNC_MODE = %d\n", 
                ucc_mode_param, UCC_UART_SYNC_MODE);
...
//#ifndef __ASYNC_UART__
         if (UCC_UART_SYNC_MODE == qe_port->ucc_mode)
         {
              ...
              * Allocate the memory
              */
             printk("DEBUG: ioremapping SCFG_QEIOCLKCR_REG (0x%x)...\n", SCFG_QEIOCLKCR_REG);
             qe_port->scfg_base = ioremap(SCFG_QEIOCLKCR_REG, 0x10);
             printk("DEBUG: ioremap returned: %p\n", qe_port->scfg_base);
             ...
             printk("DEBUG: UCC1_BASE_REG = 0x%llx, UCC3_BASE_REG = 0x%llx\n", 
                    (unsigned long long)UCC1_BASE_REG, (unsigned long long)UCC3_BASE_REG);
             printk("DEBUG: Current res.start = 0x%llx\n", (unsigned long long)res.start);

             if (UCC1_BASE_REG == res.start)
             {
                 printk("DEBUG: Matched UCC1 base address\n");
                 /* 
                  * Clear the BRG01 bits
                  */
                 reg_val = reg_val & (~ENABLE_BRG01_CLK_MASK);
                 ...
                 *(unsigned int*)qe_port->scfg_base = reg_val;
                 printk("SyncUCCUART : Enabling UCC1 BRG01-TXCLK\n");
                 printk("DEBUG: New register value: 0x%x\n", reg_val);

                 qe_port->ucc_port = 1;
                 printk("DEBUG: ucc_port set to: %d\n", qe_port->ucc_port);
             }
             else if (UCC3_BASE_REG == res.start)
             {
                 printk("DEBUG: Matched UCC3 base address\n");
                 /* 
                  * Clear the BRG02 bits
                  */
                 reg_val = reg_val & (~ENABLE_BRG02_CLK_MASK);
            ...
            {
                printk("DEBUG: No UCC base match found\n");
            }
            printk("SyncUCCUART : %s QE IO Clock Control Register (SCFG_QEIOCLKCR) Set to Zero = %x\n", 
                   __func__, *(unsigned int*)qe_port->scfg_base);
        }
        else
        {
            printk("UCC Configuring Asynchronous UART **********\n");
             if (UCC1_BASE_REG == res.start)
             {
                 printk("DEBUG: Matched UCC1 base address for async\n");
                 qe_port->ucc_port = 1;
             }
             else if (UCC3_BASE_REG == res.start)
             {
                 printk("DEBUG: Matched UCC3 base address for async\n");
                 qe_port->ucc_port = 3;
             }
             else
             {
                 printk("DEBUG: No UCC base match found for async\n");
             }

            qe_port->ucc_mode = UCC_UART_ASYNC_MODE;
            printk("DEBUG: ucc_port set to: %d, mode set to ASYNC\n", qe_port->ucc_port);
        }
//#endif
	/* Get the UCC number (device ID) */
	/* UCCs are numbered 1-7 */
	printk("DEBUG: Reading UCC number from device tree...\n");
	ret = of_property_read_u32_index(np, "cell-index", 0, &val);
	printk("DEBUG: cell-index read returned: %d, val=%u\n", ret, val);
	if (ret) {
		ret = of_property_read_u32_index(np, "device-id", 0, &val);
		printk("DEBUG: device-id read returned: %d, val=%u\n", ret, val);
		if (ret) {
			dev_err(&ofdev->dev, "UCC is unspecified in "
				"device tree\n");
			printk("ERROR: Neither cell-index nor device-id found\n");
			ret = -EINVAL;
			goto out_free;
		}
	}

	printk("DEBUG: UCC number (raw) = %u\n", val);
	if ((val < 1) || (val > UCC_MAX_NUM)) {
		dev_err(&ofdev->dev, "no support for UCC%u (max=%u)\n", val, UCC_MAX_NUM);
		printk("ERROR: UCC number out of range\n");
		ret = -ENODEV;
		goto out_free;
	}
	qe_port->ucc_num = val - 1;
	printk("DEBUG: qe_port->ucc_num = %u (UCC%u)\n", qe_port->ucc_num, qe_port->ucc_num + 1);
	...
	printk("DEBUG: Reading rx-clock-name...\n");
	sprop = of_get_property(np, "rx-clock-name", NULL);
	if (!sprop) {
		dev_err(&ofdev->dev, "missing rx-clock-name in device tree\n");
		printk("ERROR: rx-clock-name missing\n");
		ret = -ENODEV;
		goto out_free;
	}
	printk("DEBUG: Original rx-clock-name = %s\n", sprop);
        ...
        if (UCC_UART_SYNC_MODE == qe_port->ucc_mode)
        {
             printk("DEBUG: Sync mode - overriding clock names\n");
             if (UCC1_BASE_REG == res.start)
             {
                 sprop = u1clk;
                 printk("DEBUG: Using UCC1 clock: %s\n", sprop);
             }
             else
             {
                 sprop = u3clk;
                 printk("DEBUG: Using UCC3 clock: %s\n", sprop);
             } 
        }

	qe_port->us_info.rx_clock = qe_clock_source(sprop);
	printk("DEBUG: RX clock source = %d (%s)\n", qe_port->us_info.rx_clock, sprop);

#ifdef LOOPBACK
	printk("DEBUG: LOOPBACK mode enabled\n");
	/* In internal loopback mode, TX and RX must use the same clock */
	qe_port->us_info.tx_clock = qe_port->us_info.rx_clock;
	printk("DEBUG: TX clock set to same as RX: %d\n", qe_port->us_info.tx_clock);
#else
	printk("DEBUG: Reading tx-clock-name...\n");
	sprop = of_get_property(np, "tx-clock-name", NULL);
	if (!sprop) {
		dev_err(&ofdev->dev, "missing tx-clock-name in device tree\n");
		printk("ERROR: tx-clock-name missing\n");
		ret = -ENODEV;
		goto out_free;
	}
	printk("DEBUG: tx-clock-name = %s\n", sprop);
	qe_port->us_info.tx_clock = qe_clock_source(sprop);
	printk("DEBUG: TX clock source = %d\n", qe_port->us_info.tx_clock);
#endif

	/* Get the port number, numbered 0-3 */
	printk("DEBUG: Reading port-number...\n");
	ret = of_property_read_u32_index(np, "port-number", 0, &val);
	printk("DEBUG: port-number read returned: %d, val=%u\n", ret, val);
	if (ret) {
		dev_err(&ofdev->dev, "missing port-number in device tree\n");
		printk("ERROR: port-number missing\n");
		ret = -EINVAL;
		goto out_free;
	}
	qe_port->port.line = val;
	printk("DEBUG: port line set to: %u\n", qe_port->port.line);
	if (qe_port->port.line >= UCC_MAX_UART) {
		dev_err(&ofdev->dev, "port-number must be 0-%u (got %u)\n",
			UCC_MAX_UART - 1, qe_port->port.line);
		printk("ERROR: port line out of range\n");
		ret = -EINVAL;
		goto out_free;
	}

	printk("DEBUG: Parsing and mapping IRQ...\n");
	qe_port->port.irq = irq_of_parse_and_map(np, 0);
	printk("DEBUG: irq_of_parse_and_map returned: %u\n", qe_port->port.irq);
	if (qe_port->port.irq == 0) {
		dev_err(&ofdev->dev, "could not map IRQ for UCC%u\n",
		       qe_port->ucc_num + 1);
		printk("ERROR: IRQ mapping failed\n");
		ret = -EINVAL;
		goto out_free;
	}
	 ...
	 * node, but we still need to support older device trees.
	 */
	printk("DEBUG: Looking for QE node...\n");
	np = of_find_compatible_node(NULL, NULL, "fsl,qe");
	if (!np) {
		printk("DEBUG: fsl,qe not found, trying type 'qe'\n");
		np = of_find_node_by_type(NULL, "qe");
		if (!np) {
			dev_err(&ofdev->dev, "could not find 'qe' node\n");
			printk("ERROR: QE node not found\n");
			ret = -EINVAL;
			goto out_free;
		}
	}
	printk("DEBUG: QE node found: %pOF\n", np);

	printk("DEBUG: Reading brg-frequency...\n");
	ret = of_property_read_u32_index(np, "brg-frequency", 0, &val);
	printk("DEBUG: brg-frequency read returned: %d, val=%u\n", ret, val);
	if (ret) {
		printk("DEBUG: brg-frequency not found or error, trying fallback\n");
		dev_err(&ofdev->dev,
		       "missing brg-frequency in device tree\n");
		ret = -EINVAL;
		goto out_np;
	...
	if (val) {
		qe_port->port.uartclk = val;
		printk("DEBUG: Using brg-frequency = %lu Hz\n", qe_port->port.uartclk);
	} else {
		/*
		 * Older versions of U-Boot do not initialize the brg-frequency
		 * property, so in this case we assume the BRG frequency is
		 * half the QE bus frequency.
		 */
		printk("DEBUG: brg-frequency is 0, trying bus-frequency\n");
		ret = of_property_read_u32_index(np, "bus-frequency", 0, &val);
		printk("DEBUG: bus-frequency read returned: %d, val=%u\n", ret, val);
		if (ret) {
			dev_err(&ofdev->dev,
				"missing QE bus-frequency in device tree\n");
			printk("ERROR: No bus-frequency found\n");
			ret = -EINVAL;
			goto out_np;
		}
		if (val) {
			qe_port->port.uartclk = val / 2;
			printk("DEBUG: Using bus-frequency/2 = %lu Hz\n", qe_port->port.uartclk);
		} else {
			dev_err(&ofdev->dev,
				"invalid QE bus-frequency in device tree\n");
			printk("ERROR: bus-frequency is 0\n");
			ret = -EINVAL;
			goto out_np;
		}
	}

	printk("DEBUG: Final uartclk = %lu Hz\n", qe_port->port.uartclk);

	printk("DEBUG: Initializing port structure...\n");
	spin_lock_init(&qe_port->port.lock);
	qe_port->np = np;
	qe_port->port.dev = &ofdev->dev;
	qe_port->port.ops = &qe_uart_pops;
	...
	printk("DEBUG: us_info: ucc_num=%u, regs=0x%llx, irq=%u\n",
	       qe_port->us_info.ucc_num, (unsigned long long)qe_port->us_info.regs,
	       qe_port->us_info.irq);
	printk("DEBUG: rx_bd_ring_len=%u, tx_bd_ring_len=%u\n",
	       qe_port->us_info.rx_bd_ring_len, qe_port->us_info.tx_bd_ring_len);

	/* Add the port to the uart sub-system.  This will cause
	 * qe_uart_config_port() to be called, so the us_info structure must
	 * be initialized.
	 */
	printk("DEBUG: Adding port to uart subsystem...\n");
	ret = uart_add_one_port(&ucc_uart_driver, &qe_port->port);
	printk("DEBUG: uart_add_one_port returned: %d\n", ret);
	if (ret) {
		dev_err(&ofdev->dev, "could not add /dev/ttyQE%u\n",
		       qe_port->port.line);
		printk("ERROR: uart_add_one_port failed with %d\n", ret);
		goto out_np;
	}
	...
	printk("==================================================\n");
	printk("UCC UART PROBE SUCCESSFUL for /dev/ttyQE%u\n", qe_port->port.line);
	printk("==================================================\n\n");

	return 0;

out_np:
	printk("DEBUG: Error path - out_np label\n");
	of_node_put(np);
out_free:
	printk("DEBUG: Error path - out_free label\n");
	printk("DEBUG: Freeing qe_port structure at %p\n", qe_port);
	kfree(qe_port);
	printk("==================================================\n");
	printk("UCC UART PROBE FAILED with error: %d\n", ret);
	printk("==================================================\n\n");
	return ret;
}
